  ...
  public class PlaneScene : IVMR9PresentCallback, IRenderLayer
  {
    ...
    public static void RenderFor3DMode(GUIGraphicsContext.eRender3DModeHalf renderModeHalf, float timePassed, Surface backbuffer, Surface surface, Rectangle targetRect)
    {
        if (GUIGraphicsContext.Render3DMode != GUIGraphicsContext.eRender3DMode.SideBySideFrom2D ||
            GUIGraphicsContext.Render3DMode == GUIGraphicsContext.eRender3DMode.SideBySideFrom2D && renderModeHalf == GUIGraphicsContext.eRender3DModeHalf.SBSLeft)
        {
    ...
    private void InternalPresentImage(int width, int height, int arWidth, int arHeight, bool isRepaint)
    {
      ...
      try
      {
        ...
        lock (GUIGraphicsContext.RenderModeSwitch)
        {
          ...
          {
            if (GUIGraphicsContext.Render3DMode == GUIGraphicsContext.eRender3DMode.SideBySideFrom2D) // convert 2D to 3D
            {
